  ...
  return (
    <div className="mt-4">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center gap-3 rounded-lg border border-gray-200/60 dark:border-gray-600/60 bg-gray-50/50 dark:bg-gray-900/30 px-4 py-2.5 text-left text-sm font-semibold text-gray-800 dark:text-gray-200 hover:bg-gray-100/80 dark:hover:bg-gray-800/50 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-gray-800"
        aria-expanded={open}
      ...
      >
        ...
        <svg
          viewBox="0 0 20 20"
          aria-hidden="true"
          className={`w-5 h-5 shrink-0 text-blue-600 dark:text-blue-400 transition-transform duration-300 ease-theme motion-reduce:transition-none ${open ? 'rotate-180' : ''}`}
          fill="currentColor"
        >
          <path
            fillRule="evenodd"
      ...
      <div
        id={panelId}
        aria-hidden={!open}
        className={`grid transition-[grid-template-rows] duration-300 ease-theme motion-reduce:transition-none motion-reduce:duration-0 ${
          open ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'
        }`}
      >
        <div className="overflow-hidden min-h-0">
          <div
            className={`pt-2 transition-opacity duration-300 ease-theme motion-reduce:transition-none motion-reduce:duration-0 ${
