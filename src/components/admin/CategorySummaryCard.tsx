...
export type CategorySummaryVariant = 'predefined' | 'custom' | 'available'

export interface CategorySummaryCardProps {
  category: string
  categorySkills: Record<string, unknown> | unknown[]
  variant?: CategorySummaryVariant
}
...
const CategorySummaryCard = ({
  ...
  categorySkills,
  variant = 'predefined',
}: CategorySummaryCardProps): JSX.Element => {
  const isHierarchical = !Array.isArray(categorySkills)
  const subcategories = isHierarchical ? Object.keys(categorySkills) : []
  const totalSkills = isHierarchical
    ? (Object.values(categorySkills) as unknown[]).flat().length
    : (categorySkills as unknown[]).length
  ...
  const getVariantStyles = (): {
    ...
    switch (variant) {
      case 'predefined':
        return {
          container:
            'bg-blue-50 dark:bg-blue-900/30 p-2 rounded-md border border-blue-200 dark:border-blue-700',
          title: 'font-medium text-gray-700 dark:text-gray-300 mb-1',
          subcategory:
            'inline-block bg-blue-100 dark:bg-blue-800/50 text-blue-700 dark:text-blue-300 px-2 py-1 rounded text-xs mr-1 flex-shrink-0',
        }
      case 'custom':
        return {
          container:
            'bg-yellow-50 dark:bg-yellow-900/30 p-2 rounded-md border border-yellow-200 dark:border-yellow-700',
          title: 'font-medium text-gray-700 dark:text-gray-300 mb-1',
          subcategory:
            'inline-block bg-yellow-100 dark:bg-yellow-800/50 text-yellow-700 dark:text-yellow-300 px-2 py-1 rounded text-xs mr-1 flex-shrink-0',
        }
      case 'available':
        return {
          container: 'bg-gray-50 dark:bg-gray-800 p-2 rounded-md',
          title: 'font-medium text-gray-700 dark:text-gray-300 mb-1',
          subcategory:
            'inline-block bg-green-100 dark:bg-green-800/50 text-green-700 dark:text-green-300 px-2 py-1 rounded text-xs mr-1 flex-shrink-0',
        }
      default:
        return {
          container:
  ...
  const checkFit = useCallback(() => {
    ...
    const subcategoryElements = container.querySelectorAll<HTMLSpanElement>(
      'span[data-subcategory]'
    )

    const moreIndicatorWidth = 80
    const margin = 4
              ...
              {subcategories
                ...
                .map((subcategory) => (
                  ...
                  >
                    {subcategory}{' '}
                    {Array.isArray(
                      (categorySkills as Record<string, unknown[]>)[subcategory]
                    )
                      ? `(${
